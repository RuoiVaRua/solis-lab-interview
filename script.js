...
const accordion = document.querySelector('.' + classNames.containerClassName);
...
function toggleBlock(index) {
	if (accordion?.children?.length && accordion.children[index]) {
		...
		// If you click on the 'X' button of an open block, 
...
function handleOpenBlock(element, xmarkOrText = openElementType.xmark) {
	const otherOpen = document.querySelector('.' + classNames[xmarkOrText + suffixOpenClassName]);
	if (otherOpen) {
		otherOpen.classList.remove(classNames[xmarkOrText + suffixOpenClassName]);
	}
...
blocksData.forEach((data, ind) => {
	accordion.innerHTML += createBlock(data, ind);
});